...
export async function build() {
  ...
  const [template, styles, core, workflow, demoEngine, methods, researchForge, evidence, artifacts, galaxy, inquiryClient, app] = await Promise.all([
    ...
    readFile(path.join(sourceRoot, 'artifacts.mjs'), 'utf8'),
  ...
  const script = [core, workflow, demoEngine, methods, researchForge, evidence, artifacts, galaxy, inquiryClient, app].map(inlineModule).join('\n\n');
  const html = template
